...
bool BVHNode::is_valid() const {
  if (is_leaf) {
    return box.min.length() <= box.max.length() && left_idx == -1 &&
           right_idx == -1 && n_objs > 0 && n_objs < max_leaf_capacity;
  }

  return box.min.length() <= box.max.length() && n_objs == 0 && left_idx > 0 &&
         right_idx > 0 && left_idx != right_idx;
}
