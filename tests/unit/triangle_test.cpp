...
Triangle make_triangle_xy() {
  ...
  return Triangle(Vector3D(0, 0, 0), Vector3D(1.0f, 0, 0),
                  Vector3D(0, 1.0f, 0));
}
// === ТЕСТЫ ДЛЯ intersection() ===
TEST(TriangleIntersection, CoplanarFullOverlap) {
  Triangle t1 = make_triangle_xy();
  Triangle t2(Vector3D(0.2, 0.0, 0), Vector3D(1.0f, 0.0, 0),
              Vector3D(0.2, 0.8, 0));
...
TEST(TriangleIntersection, CoplanarPartialOverlap) {
  Triangle t1 = make_triangle_xy();
  Triangle t2(Vector3D(0.5, -0.2, 0), Vector3D(1.0, -0.2, 0),
              Vector3D(0.5, 0.5, 0));
  EXPECT_TRUE(t1.is_intersect(t2));
}
...
TEST(TriangleIntersection, CoplanarTouchAtVertex) {
  Triangle t1 = make_triangle_xy();
  Triangle t2(Vector3D(1.0, 0.0, 0), Vector3D(2.0, 0.0, 0),
              Vector3D(1.0, 1.0, 0));
  EXPECT_TRUE(t1.is_intersect(t2));
}
...
TEST(TriangleIntersection, CoplanarTouchAlongEdge) {
  Triangle t1 = make_triangle_xy();
  Triangle t2(Vector3D(0.0, 1.0, 0), Vector3D(1.0, 0.0, 0),
              Vector3D(1.0, 1.0, 0));
  EXPECT_TRUE(t1.is_intersect(t2));
}
...
TEST(TriangleIntersection, CoplanarDisjoint) {
  Triangle t1 = make_triangle_xy();
  Triangle t2(Vector3D(2.0, 0.0, 0), Vector3D(3.0, 0.0, 0),
              Vector3D(2.0, 1.0, 0));
...
TEST(TriangleIntersection, NonCoplanarShouldFail) {
  Triangle t1 = make_triangle_xy();
  Triangle t2(Vector3D(0, 0, 1), Vector3D(1, 0, 1), Vector3D(0, 1, 1));
  EXPECT_FALSE(t1.is_intersect(t2));
}
...
TEST(SectionOppositeDirection, CollinearOppositeDirectionDisjoint) {
  Section s1(Vector3D(0, 0, 0), Vector3D(1, 0, 0));
  Section s2(Vector3D(2 + math::eps * 10, 0, 0),
             Vector3D(3, 0, 0));  // разрыв > eps
...
TEST(SectionOppositeDirection, AlmostTouchButOutsideEps) {
  Section s1(Vector3D(0, 0, 0), Vector3D(1, 0, 0));
  Section s2(Vector3D(1 + math::eps * 10, 0, 0),
             Vector3D(2 + math::eps * 10, 0, 0));
...
TEST(Triangle3DTest, ParallelNoIntersection) {
  ...
  Triangle t2(Vector3D(0, 0, 1), Vector3D(1, 0, 1),
              Vector3D(0, 1, 1));  // параллельный, выше
  EXPECT_FALSE(t1.is_intersect(t2));
}
...
TEST(Triangle3DTest, IntersectBySegment) {
  ...
  Triangle t2(Vector3D(1, -1, -1), Vector3D(1, 1, -1), Vector3D(1, 0, 1));
...
TEST(Triangle3DTest, IntersectAtPointOnly) {
  Triangle t1(Vector3D(0, 0, 0), Vector3D(2, 0, 0), Vector3D(0, 2, 0));
  Triangle t2(Vector3D(2, 0, 0), Vector3D(2, 1, 1),
              Vector3D(2, -1, 1));  // пересекаются в точке (2,0,0)
  EXPECT_TRUE(t1.is_intersect(t2));
}
...
TEST(Triangle3DTest, ContainedButDifferentPlanes) {
  ...
  Triangle t2(Vector3D(1, 1, 1), Vector3D(2, 1, 1),
              Vector3D(1, 2, 1));  // полностью выше
  EXPECT_FALSE(t1.is_intersect(t2));
}
...
TEST(Triangle3DTest, TouchAtSingleVertex) {
  ...
  Triangle t2({0, 0, 0}, {0, 1, -1},
              {1, 0, -1});  // пересекается только в (0,0,0)
  EXPECT_TRUE(t1.is_intersect(t2));
}
...
Triangle make_point_triangle(const Vector3D& p) { return Triangle{p, p, p}; }

// Коллинеарный треугольник: вершины на одной прямой
Triangle make_segment_triangle(const Vector3D& a, const Vector3D& b,
                               const Vector3D& c) {
...
TEST(TriangleDegenerateTest,
     SegmentTriangleIsSectionNotPoint_DistinctVertices) {
  // Вершины на оси X: (0,0,0)–(1,0,0)–(2,0,0)
  Triangle t =
      make_segment_triangle({0.f, 0.f, 0.f}, {1.f, 0.f, 0.f}, {2.f, 0.f, 0.f});
...
TEST(TriangleDegenerateTest, SegmentTriangleIsSectionWhenTwoVerticesCoincide) {
  ...
  Triangle t =
      make_segment_triangle({0.f, 0.f, 0.f}, {0.f, 0.f, 0.f}, {1.f, 0.f, 0.f});
...
TEST(TriangleDegenerateIntersection,
     PointPointDifferentLocationDoNotIntersect) {
...
TEST(TriangleDegenerateIntersection, PointOnSegmentTriangleIntersect) {
  ...
  Triangle seg =
      make_segment_triangle({0.f, 0.f, 0.f}, {2.f, 0.f, 0.f}, {1.f, 0.f, 0.f});
...
TEST(TriangleDegenerateIntersection, CollinearOverlappingSegmentsIntersect) {
  ...
  // Второй: [1,3] по оси X
  Triangle s2 =
      make_segment_triangle({1.f, 0.f, 0.f}, {3.f, 0.f, 0.f}, {2.f, 0.f, 0.f});
...
TEST(TriangleDegenerateIntersection, CollinearDisjointSegmentsDoNotIntersect) {
  // Первый: [0,1]
  Triangle s1 =
      make_segment_triangle({0.f, 0.f, 0.f}, {1.f, 0.f, 0.f}, {0.5f, 0.f, 0.f});
  // Второй: [2,3]
  Triangle s2 =
      make_segment_triangle({2.f, 0.f, 0.f}, {3.f, 0.f, 0.f}, {2.5f, 0.f, 0.f});
...
TEST(TriangleDegenerateIntersection, SkewSegmentsIn3DDoNotIntersect) {
  ...
  Triangle s2 =
      make_segment_triangle({0.f, 0.f, 1.f}, {0.f, 1.f, 1.f}, {0.f, 0.5f, 1.f});
...
TEST(TriangleDegenerateIntersection, SegmentCrossesProperTriangle) {
  ...
  Triangle tri({0.f, 0.f, 0.f}, {2.f, 0.f, 0.f}, {0.f, 2.f, 0.f});

  // Треугольник-отрезок: от (1, -1, 0) до (1, 3, 0) (пересекает tri по высоте)
  Triangle seg =
      make_segment_triangle({1.f, -1.f, 0.f}, {1.f, 3.f, 0.f}, {1.f, 0.f, 0.f});
...
TEST(TriangleDegenerateIntersection, SegmentFarFromProperTriangleNoIntersect) {
  ...
  Triangle seg =
      make_segment_triangle({0.f, 0.f, 1.f}, {2.f, 0.f, 1.f}, {1.f, 1.f, 1.f});
