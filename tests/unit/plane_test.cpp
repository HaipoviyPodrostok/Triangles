...
TEST(PlaneTest, MatchSamePlane) {
  ...
  Plane p2({1, 2, 1},
           {0, 0, 1});  // тоже z = 1 (другая точка в этой же плоскости)
...
TEST(PlaneTest, ConstructorWithNegativeNormalGivesSamePlaneOppositeD) {
  ...
  EXPECT_TRUE(p1.get_normal().is_collinear(p2.get_normal()));
  EXPECT_NEAR(p1.get_D(), -p2.get_D(), 1e-6);
}
...
TEST(PlaneTest, SmallPerturbationsDontBreakMatch) {
  ...
  EXPECT_TRUE(
      p1.is_match(p2));  // должны считаться совпадающими при малой ошибке
}
